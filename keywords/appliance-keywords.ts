// Comprehensive appliance repair keywords for SEO and page creation

export const applianceKeywords = {
  // AC Brands
  ac: {
    brands: [
      'Blue Star AC Repair',
      'Voltas AC Repair',
      'LG AC Repair',
      'Samsung AC Repair',
      'Daikin AC Repair',
      'Carrier AC Repair',
      'Hitachi AC Repair',
      'Godrej AC Repair',
      'Whirlpool AC Repair',
      'Panasonic AC Repair',
      'O General AC Repair',
      'Haier AC Repair',
      'Lloyd AC Repair',
      'Onida AC Repair',
      'Videocon AC Repair'
    ],
    types: [
      'Split AC Repair',
      'Window AC Repair',
      'Inverter AC Repair',
      'Central AC Repair',
      'Portable AC Repair',
      'Cassette AC Repair'
    ],
    issues: [
      'AC Not Cooling',
      'AC Gas Leak',
      'AC Compressor Repair',
      'AC Fan Repair',
      'AC Remote Repair',
      'AC Water Leak',
      'AC Installation',
      'AC Maintenance',
      'AC Servicing'
    ]
  },

  // TV Brands
  tv: {
    brands: [
      'Samsung TV Repair',
      'LG TV Repair',
      'Sony TV Repair',
      'OnePlus TV Repair',
      'Mi TV Repair',
      'TCL TV Repair',
      'Panasonic TV Repair',
      'Videocon TV Repair',
      'Onida TV Repair',
      'BPL TV Repair',
      'Philips TV Repair',
      'Toshiba TV Repair',
      'Sharp TV Repair',
      'Akai TV Repair',
      'Thomson TV Repair'
    ],
    types: [
      'LED TV Repair',
      'LCD TV Repair',
      'Smart TV Repair',
      'OLED TV Repair',
      '4K TV Repair',
      'Android TV Repair',
      'Curved TV Repair',
      'Ultra HD TV Repair'
    ],
    issues: [
      'TV Not Starting',
      'TV Screen Repair',
      'TV Sound Problem',
      'TV Remote Repair',
      'TV Installation',
      'TV Mounting',
      'TV Display Issue',
      'TV Motherboard Repair'
    ]
  },

  // Washing Machine Brands
  washingMachine: {
    brands: [
      'Samsung Washing Machine Repair',
      'LG Washing Machine Repair',
      'Whirlpool Washing Machine Repair',
      'IFB Washing Machine Repair',
      'Bosch Washing Machine Repair',
      'Haier Washing Machine Repair',
      'Godrej Washing Machine Repair',
      'Panasonic Washing Machine Repair',
      'Onida Washing Machine Repair',
      'Videocon Washing Machine Repair',
      'Lloyd Washing Machine Repair',
      'Electrolux Washing Machine Repair',
      'BPL Washing Machine Repair',
      'Kenstar Washing Machine Repair',
      'Candy Washing Machine Repair'
    ],
    types: [
      'Front Load Washing Machine Repair',
      'Top Load Washing Machine Repair',
      'Semi Automatic Washing Machine Repair',
      'Fully Automatic Washing Machine Repair',
      'Inverter Washing Machine Repair',
      'Twin Tub Washing Machine Repair'
    ],
    issues: [
      'Washing Machine Not Spinning',
      'Washing Machine Water Leak',
      'Washing Machine Drain Problem',
      'Washing Machine Door Repair',
      'Washing Machine Motor Repair',
      'Washing Machine Installation',
      'Washing Machine Drum Repair'
    ]
  },

  // Refrigerator Brands
  refrigerator: {
    brands: [
      'Samsung Refrigerator Repair',
      'LG Refrigerator Repair',
      'Whirlpool Refrigerator Repair',
      'Godrej Refrigerator Repair',
      'Haier Refrigerator Repair',
      'Panasonic Refrigerator Repair',
      'Videocon Refrigerator Repair',
      'Onida Refrigerator Repair',
      'BPL Refrigerator Repair',
      'Electrolux Refrigerator Repair',
      'Lloyd Refrigerator Repair',
      'Sharp Refrigerator Repair',
      'Toshiba Refrigerator Repair',
      'Daikin Refrigerator Repair',
      'Carrier Refrigerator Repair'
    ],
    types: [
      'Single Door Refrigerator Repair',
      'Double Door Refrigerator Repair',
      'Side by Side Refrigerator Repair',
      'French Door Refrigerator Repair',
      'Mini Refrigerator Repair',
      'Inverter Refrigerator Repair',
      'Frost Free Refrigerator Repair',
      'Direct Cool Refrigerator Repair'
    ],
    issues: [
      'Refrigerator Not Cooling',
      'Refrigerator Gas Leak',
      'Refrigerator Compressor Repair',
      'Refrigerator Thermostat Repair',
      'Refrigerator Water Leak',
      'Refrigerator Installation',
      'Refrigerator Door Seal Repair'
    ]
  },

  // Microwave Brands
  microwave: {
    brands: [
      'Samsung Microwave Repair',
      'LG Microwave Repair',
      'IFB Microwave Repair',
      'Whirlpool Microwave Repair',
      'Panasonic Microwave Repair',
      'Godrej Microwave Repair',
      'Haier Microwave Repair',
      'Onida Microwave Repair',
      'Bajaj Microwave Repair',
      'Prestige Microwave Repair',
      'Morphy Richards Microwave Repair',
      'Kenwood Microwave Repair',
      'Philips Microwave Repair',
      'Toshiba Microwave Repair',
      'Sharp Microwave Repair'
    ],
    types: [
      'Solo Microwave Repair',
      'Grill Microwave Repair',
      'Convection Microwave Repair',
      'Inverter Microwave Repair',
      'Built-in Microwave Repair',
      'Over the Range Microwave Repair'
    ],
    issues: [
      'Microwave Not Heating',
      'Microwave Door Repair',
      'Microwave Turntable Repair',
      'Microwave Installation',
      'Microwave Magnetron Repair',
      'Microwave Control Panel Repair'
    ]
  },

  // Laptop Brands
  laptop: {
    brands: [
      'Dell Laptop Repair',
      'HP Laptop Repair',
      'Lenovo Laptop Repair',
      'Asus Laptop Repair',
      'Acer Laptop Repair',
      'MSI Laptop Repair',
      'Apple MacBook Repair',
      'Samsung Laptop Repair',
      'Sony Vaio Laptop Repair',
      'Toshiba Laptop Repair',
      'Fujitsu Laptop Repair',
      'Gateway Laptop Repair',
      'Compaq Laptop Repair',
      'IBM Laptop Repair',
      'HCL Laptop Repair'
    ],
    types: [
      'Gaming Laptop Repair',
      'Business Laptop Repair',
      'Ultrabook Repair',
      'Chromebook Repair',
      'Workstation Laptop Repair',
      'Budget Laptop Repair',
      'Premium Laptop Repair',
      'Convertible Laptop Repair'
    ],
    issues: [
      'Laptop Screen Repair',
      'Laptop Keyboard Repair',
      'Laptop Motherboard Repair',
      'Laptop Battery Repair',
      'Laptop Charging Port Repair',
      'Laptop Data Recovery',
      'Laptop Software Installation',
      'Laptop Hardware Upgrade'
    ]
  },

  // Mobile Phone Brands
  mobile: {
    brands: [
      'Samsung Mobile Repair',
      'iPhone Repair',
      'OnePlus Mobile Repair',
      'Xiaomi Mobile Repair',
      'Vivo Mobile Repair',
      'Oppo Mobile Repair',
      'Realme Mobile Repair',
      'Motorola Mobile Repair',
      'Nokia Mobile Repair',
      'LG Mobile Repair',
      'Sony Mobile Repair',
      'HTC Mobile Repair',
      'BlackBerry Mobile Repair',
      'Google Pixel Repair',
      'Huawei Mobile Repair'
    ],
    types: [
      'Smartphone Repair',
      'Feature Phone Repair',
      'Gaming Phone Repair',
      'Business Phone Repair',
      'Budget Phone Repair',
      'Premium Phone Repair',
      'Rugged Phone Repair',
      'Foldable Phone Repair'
    ],
    issues: [
      'Mobile Screen Repair',
      'Mobile Battery Replacement',
      'Mobile Charging Port Repair',
      'Mobile Camera Repair',
      'Mobile Speaker Repair',
      'Mobile Data Recovery',
      'Mobile Software Update',
      'Mobile Water Damage Repair'
    ]
  }
}

// Generate all possible combinations for SEO
export const generateAllKeywords = () => {
  const allKeywords = []
  
  Object.values(applianceKeywords).forEach(category => {
    if (category.brands) allKeywords.push(...category.brands)
    if (category.types) allKeywords.push(...category.types)
    if (category.issues) allKeywords.push(...category.issues)
  })
  
  return allKeywords
}

// Get keywords for a specific category
export const getKeywordsForCategory = (category: string) => {
  return applianceKeywords[category as keyof typeof applianceKeywords] || []
}

// Get WhatsApp message for specific repair
export const getWhatsAppMessage = (keyword: string) => {
  return `Hi! I need help with ${keyword.toLowerCase()} in Hyderabad. Please provide me with a quote and schedule a service.`
}
